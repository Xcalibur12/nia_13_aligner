...
SRC_DATA_PATH=$BASE_PATH/$1 # Do not change # nia_all
...
dictionary_path=$TOOL_PATH/nia_13_read_lexicon_20221221_sorted.txt
...
# Convert phone2jamo
if [ $stage -le 1 ]; then
echo "STEP 1: Apply phone2jamo to aligned TextGrids"
...
if [ $stage -le 2 ]; then
echo "STEP 2: Add additional tiers to finalize the TextGrid format"
...
if [ $stage -le 3 ]; then
echo "STEP 3: Copy TextGrids to $RESULT_PATH"
...
if [ $stage -le 4 ]; then
echo "STEP 4: Change to speaker-wise results"
python $TOOL_PATH/make_speaker_folders.py $SRC_DATA_PATH
echo "==> DONE"
echo " "
fi
